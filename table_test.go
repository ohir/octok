...
var tokTestTable = []ptestItem{ // desc, Fl, Tt, Np, pResult / from / mock
	...
	{`too many part name`, 0x00, 0x00, 0x8cca, LintKeyParts, ParseOK,
		...
		"N                   n : v^"},
	{`no parts name from the dot`, 0x00, 0x00, 0x0000, LintOK, ParseOK,
		" '.name : v$",
		"  N   n : v^"},
	{`three parts name from the space`, 0x00, 0x00, 0x04ca, LintOK, ParseOK,
		" ' name two.three : v$",
		"  N             n : v^"},
	{`three parts name from many spaces`, 0x00, 0x00, 0xa1b5, LintOK, ParseOK,
		" '   name two  three   four : v$",
		"  N                       n : v^"},
	{`four parts name from the dot`, 0x00, 0x00, 0x9950, LintOK, ParseOK,
		" '.name.two.three.four : v$",
		"  N                  n : v^"},
	{`five parts name from the dot`, 0x00, 0x00, 0x9950, LintKeyParts, ParseOK,
		" '.name.two.three.four.five : v$",
		"  N                       n : v^"},
	{`dot parted key dot lead`, 0x00, 0x00, 0x04ca, LintOK, ParseOK,
		"'.Some.Key.Here : value$",
		" N            n   V   v^"},
